...
const ChatScreen = () => {
  ...
  const handleNewChat = () => {
    ...
    setActiveChatTitle(newTitle);
    navigate('/'); // Navigate to main chat when creating a new chat
  };
  ...
  return (
    <Box sx={{ display: 'flex', fontFamily: 'Ubuntu, sans-serif' }}>
      {/* Only render the drawer when on the history route */}
      {isHistoryRoute && (
        <Drawer
          variant="permanent"
          sx={{
        ...
        >
          <Box>
            ...
            <Typography variant="subtitle1" sx={{ mb: 1, fontWeight: 'bold', px: 1 }}>
      ...
      >
        {/* AppBar for navigating between chat and history */}
        ...
        {/* Rating filter bar - only show on history page */}
        {isHistoryRoute && (
          <Box sx={{
            display: 'flex',
            alignItems: 'center',
        ...
        <Routes>
                    ...
                    <Box>
                      <Paper
                        sx={{
                          ...
                          py: 1,
                          mb: 0.5,
                          width: isHistoryRoute ? 'calc(80vw - 200px)' : '80vw'
                        }}
                      >
          ...
          <Route path="/" element={
            <>
              ...
              >
                {[...currentChat].reverse().map((item, index) => (
                  <Box key={index} sx={{ mb: 2 }}>
                    {/* User Message */}
                    <Box sx={{ display: 'flex', gap: 1, alignItems: 'flex-start', mb: 2, alignSelf: 'flex-start' }}>
                      <Box>
      ...
      <Dialog open={feedbackModalOpen} onClose={() => setFeedbackModalOpen(false)}>
        <DialogTitle sx={{ bgcolor: '#F1E8FB', color: '#6A1B9A' }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            ...
            <Typography fontWeight="bold">Soul AI Feedback</Typography>
          </Box>
        </DialogTitle>
